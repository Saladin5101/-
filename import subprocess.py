...
# ------------------- 配置区 -------------------
LOG_PATH = os.path.expanduser("~/Library/Logs/System.log")  # 改为用户可访问的日志路径
...
def get_recipient_email():
    ...
    try:
        ...
        show_error_in_main_thread("配置错误", "邮箱配置损坏，请重新设置", tk.Tk())
        input_email_gui()
        return config['alert']['recipient_email']
    ...
    def on_close():
        if messagebox.askokcancel("确认", "确定要停止监控吗？"):
            ...
            if global_process is not None:
                global_process = None  # 纯Python实现无需强制终止，重置状态即可
